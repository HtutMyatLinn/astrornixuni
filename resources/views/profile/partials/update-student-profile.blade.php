        ...
        <form action="{{ route('profile.update') }}" method="POST" enctype="multipart/form-data"
            ...
            <div class="flex justify-center mb-3">
                <div class="relative w-40 h-40 rounded-full my-3 select-none group">
                    ...
                    @else
                    <div id="profilePreview" class="w-full h-full bg-gray-300 rounded-full flex items-center justify-center text-white text-4xl font-bold uppercase">
            ...
            <div class="flex flex-col sm:flex-row gap-4">
                <div class="flex-1">
                    ...
                    <input type="text" name="username" placeholder="Enter your username" value="{{ old('username', Auth::user()->username) }}"
                        class="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring focus:ring-gray-300 text-sm md:text-base">
                </div>

                <div class="flex-1">
                    <label class="block text-sm md:text-base font-semibold">First Name<span class="text-red-500">*</span></label>
                    <input type="text" name="first_name" placeholder="Enter your first name" value="{{ old('first_name', Auth::user()->first_name) }}"
                        class="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring focus:ring-gray-300 text-sm md:text-base">
                </div>
            </div>
            ...
            <div class="flex-1">
                ...
                <input type="text" name="last_name" placeholder="Enter your last name" value="{{ old('last_name', Auth::user()->last_name) }}"
                    class="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring focus:ring-gray-300 text-sm md:text-base">
            </div>

            <hr class="my-4 border-gray-400">

            <button type="submit" class="w-full bg-[#5A7BAF] text-white py-2 md:py-2.5 rounded-md hover:bg-[#4A6A9F] transition text-sm md:text-base">
                Save Changes
            </button>
        </form>
    </div>
