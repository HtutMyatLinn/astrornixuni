    ...
    <div class="flex flex-col justify-center items-center min-h-screen py-10">
        ...
        <div id="passwordModal" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 hidden">
            <div class="bg-white rounded-xl p-8 w-full max-w-md shadow-lg">
                ...
                <form id="passwordForm" action="{{ route('profile.change-password') }}" method="POST" class="space-y-6">
                    @csrf
                    <!-- Current Password -->
                    <div class="space-y-2">
                    ...
                    <div class="space-y-2">
                        ...
                        <div id="passwordStrength" class="hidden">
                            <div class="h-1 w-full bg-gray-200 rounded-full mt-2">
                                <div id="passwordStrengthBar" class="h-1 rounded-full bg-red-500 w-0 transition-all duration-300"></div>
                            </div>
                    ...
                    <div class="space-y-2">
                        <label for="new_password_confirmation" class="text-xl font-medium">Confirm New Password</label>
                        <div class="relative">
                            <input
                                type="password"
            ...
            if (newPasswordInput) {
                newPasswordInput.addEventListener('input', function() {
                    ...
                    if (password.length > 0) {
                        ...
                        if (strength <= 25) {
                            passwordStrengthBar.className = 'h-1 rounded-full bg-red-500 transition-all duration-300';
                            passwordStrengthText.textContent = 'Password strength: Weak';
                            passwordStrengthText.className = 'text-xs mt-1 text-red-500';
                        } else if (strength <= 50) {
                            passwordStrengthBar.className = 'h-1 rounded-full bg-orange-500 transition-all duration-300';
                            passwordStrengthText.textContent = 'Password strength: Fair';
                            passwordStrengthText.className = 'text-xs mt-1 text-orange-500';
                        } else if (strength <= 75) {
                            passwordStrengthBar.className = 'h-1 rounded-full bg-yellow-500 transition-all duration-300';
                            passwordStrengthText.textContent = 'Password strength: Good';
                            passwordStrengthText.className = 'text-xs mt-1 text-yellow-600';
                        } else {
                            passwordStrengthBar.className = 'h-1 rounded-full bg-green-500 transition-all duration-300';
